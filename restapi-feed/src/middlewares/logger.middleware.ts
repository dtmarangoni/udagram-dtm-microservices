...
// Winston transports
// Transports are the storage devices for logs
const transports = {
    console: new winston.transports.Console(),
    infoFile: new winston.transports.File({
        dirname: path.join(__dirname, '../', '/log/info'),
        filename: 'server_info.log',
        // Maximum log file before creating a new one
...
function generateLogger(loggerType: 'info' | 'error') {
    if (loggerType === 'info') {
        return expressWinston.logger({
            ...
            format: formatter,
            // customize the default logging message to include the unique request id
            msg: (req, res) => {
